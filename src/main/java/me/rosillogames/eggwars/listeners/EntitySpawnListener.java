...
public class EntitySpawnListener implements Listener
{
    ...
    public void onEntitySpawn(EntitySpawnEvent event)
    {
        ...
        if (arena != null)
        {
            ...
            if (event instanceof ItemSpawnEvent && !arena.getStatus().isGame() && !arena.getStatus().equals(ArenaStatus.SETTING))
            {//blocks that need support dropped when regenArena deleted support, this is because all entities get removed BEFORE this
                event.setCancelled(true);
                return;
            }
